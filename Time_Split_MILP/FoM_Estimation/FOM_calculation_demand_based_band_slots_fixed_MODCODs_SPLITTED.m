function [RC,SC,UC,EC,TTS, Ill,B,P]=FOM_calculation_demand_based_band_slots_fixed_MODCODs_SPLITTED(B_T,N, M, P_T, Ill, B, P,c_scenario, UpC,n_users,number_cells, beams, theta, colours, frame, frame_dur, TTL, freq, P_list, D, solution_so_far, i_range, b_index, b_range, m_index, m_range, g_index, g_range, f_index, f_range, c_index, c_range, z_index, z_range, x_index, x_range,t_current)

%% Solution so far:
result=solution_so_far;
...
for u=1:n_users    
    for t=1:frame
        for m=1:M
            for n=1:N
                if ZS(u,m,n,t)*P_list(m,n,u)>0
                    PS(u,t)=PS(u,t)+ZS(u,m,n,t)*P_list(m,n,u); % iT SHOULD ENTER HERE ONLY FOR A SINGLE PAIR M,N
                end
            end
        end
    end
...
Ill_in=Ill_out(:,1:frame_current);
B_in=B_out(:,:,1:frame_current);
...
for t=1:frame %1frame=10 subframe by 10subframe (beamswithching is performed each frame, for 10 slots constant illumination constant)
    ...
    if t>frame_current % DECISSION TO BE TAKEN BY DB
        % % PAINT
    %     % Map plot per slot:
    ...
    %                     % Extra_losses.Ag; Extra_losses.Ac; Extra_losses.Ar; Extra_losses.As; Extra_losses.At;
                        ...
                        [minDistance, indexOfMin] =min(abs(10^(P_tx/10)-P_list(:,b_length,c_scenario(idx_max2min(beam_idx)).users(u_idx).id)));
                        if P_list(indexOfMin,b_length,c_scenario(idx_max2min(beam_idx)).users(u_idx).id)>10^(P_tx/10)
                            indexOfMin=indexOfMin-1;
                        end
                        if indexOfMin==0
                            break
                        end
                        P_tx=10*log10(P_list(indexOfMin,b_length,c_scenario(idx_max2min(beam_idx)).users(u_idx).id));
    ...
    else % DECISSION ALREADY TAKEN BY ANALYTICAL APPROACH
